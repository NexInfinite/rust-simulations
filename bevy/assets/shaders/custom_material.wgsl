...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
	let gap = 50.0 / zoom;  // Known bug where lines are drawn wrong on some prime numbers
	let real_coords = floor(mesh.uv.xy * window_size) - vec2(0.5) * window_size;
	let m = abs(real_coords % vec2(gap));
	let fragColor = vec4(vec3(1.0) - vec3(1.0) * clamp(min(m.x, m.y), clamp(min(abs(real_coords.x), abs(real_coords.y)), 0.7, 0.95), 1.0), 1.0);
	return fragColor;
}
